  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-5 px-5 py-10">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black text-yellow-400">🏆 BingoChain Volume Cup</h1>
      ...
      <div className="rounded-2xl border border-neutral-800 bg-neutral-900 p-5 text-sm text-neutral-300">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-neutral-400">
          <li>Play BingoChain arenas staked in <span className="text-yellow-400">$LANCE</span> and submit your settlement tx hashes.</li>
          ...
          <li>Top <span className="text-neutral-200">{c?.topN ?? 10}</span> by volume each win{" "}
            <span className="text-yellow-400">{c?.prizePerWinner ?? "20"} $LANCE</span>.</li>
        </ul>
        {c?.claudelanceUrl ? (
          <p className="mt-3 text-xs text-neutral-500">
            On-chain bounty:{" "}
            <a href={c.claudelanceUrl} target="_blank" rel="noreferrer" className="text-yellow-400 underline-offset-2 hover:underline">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {c.participants.map((p) => (
                  <tr key={p.address} className={`border-t border-neutral-800 ${p.won ? "bg-yellow-400/[0.06]" : ""}`}>
